...
export const CursorTraitsMenuContent = memo(function CursorTraitsMenuContentImpl({
  ...
  const setModel = useComposerDraftStore((s) => s.setModel);
  const setStickyModel = useComposerDraftStore((s) => s.setStickyModel);
  ...
  const applyNextSelection = useCallback(
    (nextSel: typeof selection) => {
      ...
      setModel(threadId, nextSel.family);
      setProviderModelOptions(threadId, "cursor", persisted, { persistSticky: true });
      setStickyModel(nextSel.family);
    },
    [setModel, setProviderModelOptions, setStickyModel, threadId],
  );
