  ...
  return Effect.gen(function* () {
    ...
    const startSession: CursorAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const cursorOpts = input.providerOptions?.cursor;
        const initialModel = resolveCursorDispatchModel(input.model, input.modelOptions?.cursor);
    ...
    const sendTurn: CursorAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const model = resolveCursorDispatchModel(
          input.model ?? ctx.session.model,
          input.modelOptions?.cursor,
        );
        ...
        if (model !== activeModel) {
          ...
          yield* startSession({
            ...
            model,
            ...(ctx.spawnOptions ? { providerOptions: { cursor: ctx.spawnOptions } } : {}),
